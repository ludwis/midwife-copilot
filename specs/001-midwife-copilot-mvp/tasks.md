...
- [ ] T003 [P] Initialize frontend: `frontend/package.json` with `vue@3`, `vite`, `vite-plugin-pwa`, `pinia`, `vue-router`, `tailwindcss`, `typescript`; create `frontend/tsconfig.json` and `frontend/tailwind.config.ts`
...
- [ ] T036 [P] [US2] Create `bot/onboarding.py`: `generate_invite(client_id: str, phone_number: str) -> (token: str, link: str)` — generates UUID4 token, builds `wa.me/{WHATSAPP_PHONE_NUMBER_ID}?text={token}` link; `CONSENT_PROMPT_TEXT` constant with AI disclosure message; `async send_consent_prompt(phone_number: str)` calls `adapters/whatsapp.send_message` with disclosure + "Reply YES to proceed"
...
- [ ] T040 [US2] Extend `process_inbound` in `api/routes/webhook.py`: after client lookup — if client not found by phone number, check `clients` collection for matching `invite_token` in message body; on match: link phone to client, set state=`awaiting_consent`, call `onboarding.send_consent_prompt`, write audit event; guard: if token already used (state != `pending_invite`), ignore
- [ ] T041 [US2] Extend `process_inbound`: if client state is `awaiting_consent` — check if body is "YES" (case-insensitive trim); on YES: use Firestore transaction `transition_to_active(client_ref, consent_data)` per research.md pattern, record `consent.message_id`, `consent.timestamp`, `consent.verbatim_reply`; on non-YES: resend consent prompt; write audit events for both paths
...
- [ ] T065 [P] Create `api/routes/ask.py` for `POST /ask`: accepts `{user_id, message, conversation_id?}`; calls `safety.classify_urgency` → `retriever.retrieve_context` → `generator.generate_draft`; returns `{draft, urgency, sources, requires_review: true}`; Bearer token protected; does NOT call `send_message` (co-pilot gate always open)
...
- **Constitution I enforced in tasks**: `adapters/whatsapp.send_message` is called ONLY from `api/routes/admin.py` reply handler (T027) and `bot/onboarding.py` (T036 — consent prompt only); never from `bot/` processing pipeline
