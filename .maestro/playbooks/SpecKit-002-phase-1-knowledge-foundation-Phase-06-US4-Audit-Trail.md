...
- [x] T052 [US4] Verify `kb_chunk_approved`, `kb_chunk_edited`, `kb_chunk_discarded`, `kb_chunk_promoted` are emitted in `backend/api/admin/kb/chunks.py` (`kb_chunk_edited` MUST include `content_hash_before` and `content_hash_after` per data-model.md; add any missing `audit.write_event()` calls for each action branch)
  <!-- Done: kb_chunk_discarded and kb_chunk_edited were already present and complete. Added content_hash to kb_chunk_approved. Added kb_chunk_promoted events to both the approve branch (with content_hash + production_vertex_id) and the edit_approve branch (with new_hash + production_vertex_id). test_audit_log.py now passes green — all 8 event types asserted including ≥2 kb_chunk_promoted events. -->
- [x] T053 [US4] Verify `kb_query_test` audit event is emitted in `backend/api/admin/kb/production.py` (fields: `query_text`, `result_count`; add missing `audit.write_event()` call if absent)
  <!-- Done: production.py lines 94–99 already emit kb_query_test with actor="admin", query_text=q, result_count=len(results) — all required fields present. No changes needed; field completeness confirmed. -->
- [x] T054 [US4] Document production GCS retention lock procedure in `cloudbuild.yaml` as a commented-out step (commands: `gsutil mb -l europe-west1 gs://midwife-bot-audit-prod`, `gsutil retention set 7y`, `gsutil retention lock`; add `# IRREVERSIBLE — run once on production project only` warning; keep dev bucket creation as an active step for CI; add `AUDIT_BUCKET_NAME` substitution variable)
  <!-- Done: Added substitutions._AUDIT_BUCKET_NAME=midwife-bot-audit-dev, active create-audit-bucket-dev step (gsutil mb || true), and commented-out production block with IRREVERSIBLE warning and all 3 gsutil commands. -->
...
- [x] `INTEGRATION=true pytest backend/tests/integration/test_vertex_search.py` passes (requires real GCP dev project)
  <!-- Verified 2026-05-16: 2 passed in 7.64s. Fixed three issues: (1) google-cloud-discoveryengine package was not installed in .venv — installed v0.19.0; (2) DocumentServiceAsyncClient was using the default global endpoint — added _client_options() helper to set eu-discoveryengine.googleapis.com for non-global locations; (3) doc.struct_data returns a proto-plus MapComposite (dict-like) not a protobuf Struct — changed fields = doc.struct_data.fields + .string_value accessors to fields = dict(doc.struct_data) + direct value access. Data stores midwife-staging and midwife-production created in meant4-intelligence project (eu region). -->
- [x] Run `/speckit-analyze` to verify consistency
  <!-- Done 2026-05-16: 0 critical, 0 high issues. 2 medium: C1 (SC-002 has no timing instrumentation — acceptable manual benchmark during T058), U1 (SC-005 sign-off section in compliance doc must be populated before T059 checks it). 3 low: A1 (FR-012 threshold not in spec), C2 (SC-007 semantic quality only manually validated), I1 (kb_chunk_promoted ×2 vs spec's "promote one" — test is correct). 100% FR and SC coverage across all 59 tasks. No constitution violations. -->
