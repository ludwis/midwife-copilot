...
- [ ] T051 [US4] Verify `kb_chunk_staged` audit event fields in `backend/bot/kb/staging.py` match data-model.md audit schema (`import_id`, `chunk_id`, `content_hash`, `duplicate_flag`; the call itself is implemented in T031 — this task confirms field completeness and that the event appears in `test_audit_log.py` output)
